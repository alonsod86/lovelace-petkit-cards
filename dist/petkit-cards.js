...
PetkitLitterboxCommandsControl = __decorateClass$8([
...
PetkitLitterboxCard = __decorateClass$7([
...
let PetkitLitterboxTimelineCard = class extends i$2 {
  ...
  _renderVertical(sourceEvents, labelColorMap) {
    ...
    return b`
      <div class="timeline-v">
        ${events.map(
      (ev, i4) => this._renderVerticalItem(ev, i4 === events.length - 1, labelColorMap)
    )}
      </div>
    `;
  }
   ...
   */
  _renderMark(ev, _meta) {
    const icon = ev.source === "primary" ? this._config.entity_icon : this._config.secondary_entity_icon;
    if (icon?.trim()) {
      return b`<ha-icon
  ...
  _renderHorizontal(sourceEvents, labelColorMap) {
    ...
    return b`
      <div class="timeline-h">
        ${events.map(
      (ev, i4) => this._renderHorizontalItem(ev, i4, events.length, labelColorMap)
    )}
      </div>
    `;
  }
  _renderHorizontalItem(ev, index, total, labelColorMap) {
    const meta = getStateMeta(ev.state);
    const labelColor = labelColorMap.get(ev.state);
    ...
    return b`
      <div class="h-item ${meta.cssClass}"
        ...
        <div class="h-content">
          ...
          ${showTime ? b`<span class="h-time">${formatTime(ev.startTime)}</span>` : A}
          ${showDuration ? b`<span class="h-dur">${formatDuration(ev.durationSeconds)}</span>` : A}
        </div>
      </div>
    `;
  }
...
PetkitLitterboxTimelineCard = __decorateClass$6([
...
const DEFAULT_STATE_ICON$1 = "mdi:help-circle-outline";
const DEFAULT_STATE_RGB$1 = "158, 158, 158";
const ACTIVE_STATES$1 = /* @__PURE__ */ new Set([
  "cleaning",
  "scooping",
  "dumping",
  "leveling",
...
const CIRCUMFERENCE$1 = 2 * Math.PI * 26;
function ringDash$1(pct) {
  const filled = Math.min(100, Math.max(0, pct));
  return `${(CIRCUMFERENCE$1 * filled / 100).toFixed(2)} ${CIRCUMFERENCE$1.toFixed(2)}`;
}
...
let PetkitLitterboxDashboardCard = class extends i$2 {
  ...
  render() {
    ...
    const titleText = this._config.title?.trim() || stateObj?.attributes?.friendly_name || this._config.entity;
    return b`
      <ha-card>
  ...
  _renderActions() {
    const cfg = this._config;
    const btns = ["btn_1", "btn_2"].flatMap((prefix) => {
      const entity = cfg[`${prefix}_entity`];
      if (!entity) return [];
      return [{
        entity,
        name: cfg[`${prefix}_name`],
        icon: cfg[`${prefix}_icon`],
        icon_color: cfg[`${prefix}_icon_color`],
        tap_action: cfg[`${prefix}_tap_action`]
      }];
    });
    if (btns.length === 0) return A;
    return b`
  ...
  _renderActionBtn(btn) {
    const stateObj = this.hass?.states[btn.entity];
    const label = btn.name ?? stateObj?.attributes?.friendly_name ?? btn.entity;
    const icon = btn.icon ?? stateObj?.attributes?.icon ?? "mdi:gesture-tap-button";
    const iconStyle = btn.icon_color ? { color: btn.icon_color, background: `color-mix(in srgb, ${btn.icon_color} 15%, transparent)` } : {};
    return b`
      <div
        class="action-btn"
        role="button"
        tabindex="0"
        @action=${(ev) => handleAction(this, this.hass, { entity: btn.entity, tap_action: btn.tap_action ?? { action: "toggle" } }, ev.detail.action)}
        .actionHandler=${actionHandler({ hasHold: false, hasDoubleClick: false })}
      >
        <div class="action-btn-icon" style=${o(iconStyle)}>
          <ha-state-icon
            .hass=${this.hass}
            .stateObj=${stateObj}
            .icon=${icon}
  ...
  _renderArm(position, entityId) {
    ...
    const unit = stateObj.attributes.unit_of_measurement ?? "";
    const customName = position === "top" ? this._config.arm_top_name : this._config.arm_bottom_name;
  ...
  _renderStateBadge(stateObj) {
    ...
    const rgb = STATE_RGB$1[s2] ?? DEFAULT_STATE_RGB$1;
    const icon = STATE_ICONS$1[s2] ?? DEFAULT_STATE_ICON$1;
    const label = defaultLabel$1(s2);
    const isActive2 = ACTIVE_STATES$1.has(s2);
    return b`
        ...
        <div class="ring-wrap">
          <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <circle
              ...
              stroke-dasharray="${ringDash$1(pct)}"
...
PetkitLitterboxDashboardCard = __decorateClass$5([
  t$1(PETKIT_DASHBOARD_CARD_NAME)
], PetkitLitterboxDashboardCard);
const PETLIBRO_DOCKSTREAM_2_CARD_NAME = `mushroom-petlibro-dockstream-2-card`;
const PETLIBRO_DOCKSTREAM_2_CARD_EDITOR_NAME = `${PETLIBRO_DOCKSTREAM_2_CARD_NAME}-editor`;
const PETLIBRO_DOCKSTREAM_2_STATE_DOMAINS = ["sensor", "binary_sensor"];
const petlibroDockstream2CardConfigStruct = assign(
  lovelaceCardConfigStruct,
  object({
    entity: string(),
    picture: optional(string()),
...
const PETLIBRO_DOCKSTREAM_2_IMAGE_URL = "data:image/webp;base64,";
...
const MODE_STATES = /* @__PURE__ */ new Set([
...
function stateBadgeKey(s2) {
  return MODE_STATES.has(s2) ? s2 : s2.toLowerCase();
}
registerCustomCard({
  type: PETLIBRO_DOCKSTREAM_2_CARD_NAME,
  name: "Mushroom Petlibro Dockstream 2 Card",
  description: "Device hero card with sensor stats for the Petlibro Dockstream 2 Smart Fountain (PLWF106 / PLWF116 cordless)"
});
let PetlibroDockstream2Card = class extends i$2 {
  static async getConfigElement() {
    await Promise.resolve().then(() => petlibroDockstream2CardEditor);
    return document.createElement(
  ...
  _renderHero(picture) {
    ...
    if (hasArms) {
      ...
      return b`
        ...
        (a2) => b`
                <div
                  class="arm-connector arm-connector-${a2.position}"
                  aria-hidden="true"
                ></div>
                ${this._renderArm(a2.position, a2.entityId)}
              `
      )}
          </div>
        </div>
...
__decorateClass$4([
  n$1({ attribute: false })
], PetlibroDockstream2Card.prototype, "hass", 2);
__decorateClass$4([
  r()
], PetlibroDockstream2Card.prototype, "_config", 2);
PetlibroDockstream2Card = __decorateClass$4([
  t$1(PETLIBRO_DOCKSTREAM_2_CARD_NAME)
], PetlibroDockstream2Card);
console.info(
...
PetkitLitterboxCardEditor = __decorateClass$3([
...
PetkitLitterboxTimelineCardEditor = __decorateClass$2([
...
PetkitLitterboxDashboardCardEditor = __decorateClass$1([
...
PetlibroDockstream2CardEditor = __decorateClass([
  t$1(PETLIBRO_DOCKSTREAM_2_CARD_EDITOR_NAME)
], PetlibroDockstream2CardEditor);
const petlibroDockstream2CardEditor = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  get PetlibroDockstream2CardEditor() {
    return PetlibroDockstream2CardEditor;
  }
}, Symbol.toStringTag, { value: "Module" }));
